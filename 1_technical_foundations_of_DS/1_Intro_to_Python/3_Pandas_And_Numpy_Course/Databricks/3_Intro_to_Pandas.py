...
# MAGIC
# MAGIC We understand that **series** and **dataframes** are two distinct objects. They each have their own unique methods. In this section we are going to look at an example of a method, which returns different results for each of the objects.
# MAGIC
...
# MAGIC
# MAGIC Well, what happens when we use the `Series.value_counts()` method with a dataframe? First step, we should select the `sector` and `industry` columns to create a dataframe named `sectors_industries`, like this:
...
# MAGIC %md
# MAGIC Then, we'll use the `value_counts()` method:
