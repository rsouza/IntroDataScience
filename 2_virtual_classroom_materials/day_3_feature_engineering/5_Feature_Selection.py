...
# MAGIC %md
# MAGIC **Important** thing to remember is that we should perform feature selection in conjunction with the model selection
...
# MAGIC %md
# MAGIC Feature selection should be performed only on our training data to avoid overfitting. Let's split our dataset and drop our TARGET feature.
...
# MAGIC %md
# MAGIC There are 370 features in our dataset. We can now look at whether there are some constant features in X_train set using `NumPy's .var()` function, which computes the variance along with columns. Within this function we can specify argument `ddof = 1`, for more information here is the [documentation](https://numpy.org/doc/stable/reference/generated/numpy.var.html).
...
# MAGIC 
# MAGIC Quasi-constant features have very low variance (or close to 0) and contain little information, which is not useful for us. These approximately constant features won't help the ML model's performance, therefore we should consider removing them. 
# MAGIC 
# MAGIC We could filter quasi-constant features with `pandas`  in a similar way as we did with constant features, with one difference - we would set a specific threshold. Nevertheless, now we'll leave `pandas` and use rather `scikit learn` that offers a more convenient way to find quasi-constant features. 
# MAGIC 
...
# MAGIC 
# MAGIC As a first step, we define our selector for quasi-constant features, where we set the threshold = 0.01. In other words, this is the minimum value of the variance we want to have in the dataset.  
...
# MAGIC %md
# MAGIC Next, we fit 'X_train' data with 'our_selector' that find quasi-constant features.
...
# MAGIC %md
# MAGIC Using `get_support()` method, we can get all of the features we want to keep along with their names. This 'mask' we will use later to assign columns names.
# MAGIC 
# MAGIC *Note: You might wonder why we are saving feature names as `features_to_keep` variable. Scikit learn will always save the necessary state inside of the fitted transformer, but we only do it for our convience so that we can later on go back from the nameless numpy to a nice dataframe with all the column names.*
...
# MAGIC %md
# MAGIC The next step is transforming our X_train and X_test data using our_selector.
...
# MAGIC %md
# MAGIC X_train is `numpy.ndarray` object after transforming and needs to be transformed into `pandas DataFrame` again. Here, we use our created 'features_to_keep' variable to assign column names.
...
# MAGIC 
# MAGIC We are doing an ugly operation of swapping rows and columns to make use of Pandas functionality and make this operation as easy as possible. Yes, we are advising to do this with a small dataset such as ours. If you have a *big* dataset, for example, counted in TBs, you will most likely not be able to do this.
...
# MAGIC %md
# MAGIC As we can see, movies DataFrame contains 10 rows and 7 features. Now we use `.transpose()` method.
...
# MAGIC %md
# MAGIC `scikit learn` module offers many methods such as selecting features based on their importance, but we do not go there. You can find these methods in the [documentation](https://scikit-learn.org/stable/modules/feature_selection.html). Now we'll look at the correlation between features.
...
# MAGIC %md
# MAGIC Several numerical features are stored as float or integer, and one feature is stored as string data in our dataset. 
# MAGIC 
# MAGIC These numerical variables can be used to predict the quality of wine samples. So **'quality'** column is our **target feature**.  
...
# MAGIC %md
# MAGIC The datasets can also contain this type of constant feature stored as a string and have only 1 unique value/category. As this variable is not really helpful, we will drop it from the dataset. 
...
# MAGIC %md
# MAGIC Now we want to select only those features that have the variance above the threshold = 0.01. Again, we will find quasi-constant features using `scikit learn's VarianceThreshold` as we did in the previous example.
