...
# MAGIC
# MAGIC
# MAGIC 1.   [**User Guide**](https://scikit-learn.org/stable/user_guide.html). We will head over here for *thorough guidelines* within a certain topic (e.g. Decision Trees).
# MAGIC 2.   [**API**](https://scikit-learn.org/stable/modules/classes.html). We will use this whenever we want to learn about a *particular method or module* (e.g. sklearn.impute).
# MAGIC 3.   [**More > Tutorial**](https://scikit-learn.org/stable/tutorial/index.html). We can use these for some *starting tutorials* (e.g. loading artificial datasets).
...
# MAGIC %md
# MAGIC # 2. Core API
# MAGIC It is time for some hands-on exercise! Firstly, we will use the datasets module and get one of the example (toy) datasets that are available within sklearn. In this case, it will be [iris](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_iris.html).
...
# MAGIC %md
# MAGIC Let's now see if the API is really as powerful and standardized as we mentioned within the lecture. 
# MAGIC We will attempt to implement the [`MinMaxScaler`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.MinMaxScaler.html). 
# MAGIC Based on what we know, a mere (1) instantiation, (2) fit and (3) transform should be sufficient.
...
# MAGIC Your task is now to try your first transformation with sklearn.
# MAGIC Please use the same dataset (`toy_data`) and use
# MAGIC [`StandardScaler`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html#sklearn.preprocessing.StandardScaler)
# MAGIC from the preprocessing module. 
# MAGIC For now, please do not worry what exactly the transformer is doing. 
# MAGIC In a few lectures from now you will learn about it. 
# MAGIC It is important that you have your first touch with the Core API.
