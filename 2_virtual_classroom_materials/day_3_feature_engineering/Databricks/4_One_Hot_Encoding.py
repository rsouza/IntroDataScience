...
# MAGIC
# MAGIC As we already know, we can't throw the data right away into machine learning models.
# MAGIC We need to treat them in a specific way so our model's algorithm can work with them.
# MAGIC **Machine learning algorithm work with vectors of numbers**, so when it comes to values represented as a string there is an issue.
# MAGIC [`scikit learn`](https://scikit-learn.org/stable/index.html), an industry-standard library used for machine learning, does not accept categorical values represented as strings as well.
# MAGIC
# MAGIC Imagine we have categorical variables stored as string in the dataset.
# MAGIC For understanding what the encoding looks like, here's a simple example.
...
# MAGIC %md
# MAGIC Our target feature 'class' also needs to be encoded. To do so, use [`LabelEncoder`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.LabelEncoder.html).
