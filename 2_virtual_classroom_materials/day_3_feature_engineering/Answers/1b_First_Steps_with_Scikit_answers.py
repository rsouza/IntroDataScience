...
# MAGIC 
# MAGIC I presume that until now you were mainlyusing Pandas fod Data Science. As you heard within our "Introduction to Feature Engienering", there is a good reason why we should expand our library toolkit with one that is more suited for **stateful transformations** - such as *scikit-learn*. Afterwards, we are also naturally going to use it for **Machine Learning**.
# MAGIC 
# MAGIC In this notebook, we will:
# MAGIC 
# MAGIC 
# MAGIC 1.   Learn the basics about sklearn documentation
...
# MAGIC 
# MAGIC 
# MAGIC 1.   **User Guide**. We will head over here for *thorough guidelines* within certain topic (e.g. Decision Trees).
...
# MAGIC %md
# MAGIC https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.MinMaxScaler.html 
...
# MAGIC %md
# MAGIC If you scroll further down, you will see **attributes**. These are useful when we, for example, fitted the transformer and would like to examine the states that it learned.
...
# MAGIC %md
# MAGIC This looks fairly reasonable. If we look at the third column of the original data, it indeed ranged between 1.3 and 1.5. The transformer learned that its range was 0.2.

# COMMAND ----------

# MAGIC %md
# MAGIC ## TASK: Your turn
# MAGIC Your task is now to try your first transformation with sklearn. Please use the same dataset (*toy_data*) and use **StandardScaler** from preprocessing module. For now, please do not worry what exactly the transformer is doing, in a few lectures from now, you will learn about it. It is important that you have your first touch with the Core API
...
# MAGIC In the upcoming lectures, you are going to learn about a lot of individual transformations that we can perform, as well as quite a few predictive models that you can implement.  
# MAGIC 
...
# MAGIC Let's at first see how would life look like if there weren't composite estimators.  
# MAGIC 
...
# MAGIC %md
# MAGIC Even though it might not look like it from this simple example, this is really not optimal. You can imagine that we a complex dataset, the number of various transformations that we would like to employ is a lot. Notice also how we are coming up with an alternation of name of our training data after it undergoes certain transformation. This is very prone to some typo or error.  

# COMMAND ----------

# MAGIC %md
# MAGIC ## 3.2 With Advanced API: Pretty & Neat
# MAGIC The fact that under the Basic API is so standardized allowed developers to develop composite estimators. In this example, we will use one of these called **pipeline**. We will learn about more composite estimators later on.
...
# MAGIC %md
# MAGIC This is an example of how powerful the advanced API is. Notice, for example, how the first two operations are fitted without the target feature (*y_train*). You can see this by examining our ugly way, when we were fitting them individually. The third estimator however, is a supervised model that needs the target feature.  
# MAGIC 
# MAGIC The pipeline is not smart enough to provide target feature only to the estimators that need it and we do not have to worry about it, as we are providing the pipeline with both the independent features and a target feature.
