...
# MAGIC %md
# MAGIC We'll be using Titanic dataset to explore missing data in this notebook.
...
# MAGIC %md
# MAGIC We can see that three columns contain missing values: 'Age', 'Cabin' and 'Embarked'. If we want to compute the proportion of missing values, we can use `.mean()` function and plot the proportion using barplot.
...
# MAGIC %md
# MAGIC For even better visualization of Missing Values, there is a dedicated library [missingno](https://github.com/ResidentMario/missingno)
...
# MAGIC 
# MAGIC MNAR means that the probability of being missing varies for reasons that are unknown to us. Let's look at the columns 'Age' and 'Cabin' in which passengers were traveling. We found out that the column 'Cabin' contains approximately 77% missing values, the column 'Age' almost 20% missing values. 
# MAGIC 
...
# MAGIC %md
# MAGIC The output we obtained is the same as we expected. There is more missing values(approximately 87.6%) for people who did not survive compared to the survivals (60.2 %).
...
# MAGIC 
# MAGIC 
# MAGIC **Before we impute the missing values**, it is useful to mark missing values to preserve the information about which values had been missing. We can use `MissingIndicator`, which transforms the dataset into a binary variables indicating the presence of missing values (these binary variables will be added to the original training set). See the [documentation](https://scikit-learn.org/stable/modules/generated/sklearn.impute.MissingIndicator.html#sklearn.impute.MissingIndicator) for MissingIndicator.
# MAGIC 
# MAGIC In essence the idea behind missing indicator is that we **preserve extra information** for our model, which is that the value was missing. We are hoping that the model might pick up a pattern herein which we missed, for example when exploring the missing values.
# MAGIC 
...
# MAGIC We'll use Missing indicator to mark missing values by setting parameters `features = 'missing-only'` (by default). If we want to create binary variables for all features, we set parameter `features = 'all'`.
...
# MAGIC %md
# MAGIC Since the transformation of X_train returns boolean, we create a new variable to store the output. After that, we concatenate it to the original X_train.
...
# MAGIC %md
# MAGIC Now we concatenate X_train_missing boolean variables to the original X_train. To distinguish which boolean variable belongs to the original feature in X_train, we create new names (otherwise, boolean variables names will be labeled as 0,1 and 2). X_train_missing array needs to be converted using `pd.DataFrame`, since only Series and DataFrame objects are valid within `concat()` method.
...
# MAGIC Now we'll impute missing values of column Age using Simple Imputer. We specify the placeholder for missing values (np.nan) and strategy = 'mean' (this strategy is by default, so it is okay if you don't explicitly specify it within the class).
...
# MAGIC 
# MAGIC When data are missing completely at random, the probability of being missing is the same for all observations in the dataset, i.e., the cause of the missing data is unrelated to the data.
# MAGIC 
# MAGIC Let's take as an example column 'Embarked' and its missing values.
...
# MAGIC ### TASK
# MAGIC 
# MAGIC In this task, you will be using the Avocado dataset. You will impute numeric missing values in column 'Small Bags' with the median value using Simple Imputer. The second task is to impute missing values in the column 'Region' with the most frequent string value of this column using Simple Imputer.
...
# MAGIC 
# MAGIC Commonly, preparing data for machine learning models often involve several transformations such as imputing missing values, scaling numerical values, or encoding categorical features applied for particular columns. `scikit learn` offers [`ColumnTransformer`](https://scikit-learn.org/stable/modules/generated/sklearn.compose.ColumnTransformer.html) class that is used to apply different transformers to columns. This Column transformer can be chained with Pipelines along with machine learning model. You can read more about ColumnTransformer [here](https://scikit-learn.org/stable/modules/generated/sklearn.compose.ColumnTransformer.html).
